...
// ==========================================================
// MIGRAÇÃO FIREBASE → SUPABASE (T1)
// As referências ativas do Firebase (Firestore + Auth) foram desativadas no
// fluxo inicial: os dados operam localmente (localStorage) e o login Google
// via Firebase Auth está pausado. O cliente Supabase compartilhado fica em
// `lib/supabase.ts` e entra no fluxo nas tarefas T4 (auth) e T5 (dados).
// ==========================================================
import {
  findOrCreateFolder,
  createOrUpdateFile,
...
export default function Page() {
  ...
  // Sessão Supabase Auth — portão de entrada das telas internas.
  // A autorização de verdade acontece no proxy.ts (getUser, no servidor, antes
  // da página renderizar). Aqui carregamos o PERFIL (nome + papel) para a UI e
  // bloqueamos o usuário desativado, que de outro modo veria um painel vazio.
  const router = useRouter();
  const [sessao, setSessao] = useState<'carregando' | 'autenticada' | 'demo' | 'anonimo'>('carregando');
  ...
  useEffect(() => {
    ...
    const sincronizar = async (email: string | null) => {
      if (cancelado) return;

      if (!email) {
        setUsuarioEmail(null);
        setPerfil(null);
        setSessao(modoDemoAtivo() ? 'demo' : 'anonimo');
        return;
      }

      definirModoDemo(false); // login real desativa o modo demonstração
      setUsuarioEmail(email);
      setSessao('autenticada');

      const resultado = await carregarMeuPerfil();
      if (cancelado) return;
      ...
      // Sem perfil ou acesso desativado: a RLS do banco negaria todas as
      // consultas. Encerra a sessão e volta ao login, que exibe o motivo.
      setPerfil(null);
      await encerrarSessao();
      if (!cancelado) router.replace('/login');
    };

    // onAuthStateChange dispara INITIAL_SESSION imediatamente e depois cada mudança
    const { data: sub } = supabase.auth.onAuthStateChange((_evento, sessaoAtual) => {
      const email = sessaoAtual?.user?.email ?? null;
      // Fora do callback: o client de Auth segura um lock durante o evento, e
      // chamar o Supabase de dentro dele pode travar (deadlock) a requisição.
      setTimeout(() => void sincronizar(email), 0);
    });
  ...
  // Sem sessão → tela de login (rota /login)
  useEffect(() => {
    if (sessao === 'anonimo') router.replace('/login');
  }, [sessao, router]);
  ...
  // Migração Firebase → Supabase (T1): mantida inativa — só era chamada após o
  // login Google (pausado). Volta a ser usada quando o acesso ao Drive for
  // reavaliado na arquitetura Supabase (T4).
  ...
  const handleGoogleLogin = async () => {
    // Migração Firebase → Supabase (T1): o login Google via Firebase Auth foi
    // desativado. A autenticação será substituída pelo Supabase Auth (T4).
    triggerAlert(
      "Conexão Pausada",
      "O acesso ao Google Drive via login Google está pausado durante a migração Firebase → Supabase. A autenticação retorna com o Supabase Auth (T4)."
    );
  };

  const handleGoogleLogout = async () => {
    // Migração Firebase → Supabase (T1): encerramento apenas do estado local
    // (sem chamada ao Firebase Auth).
    setAccessToken(null);
    setGoogleUser(null);
    setNeedsAuth(true);
    setDriveFiles([]);
...
*Ficha exportada em: ${new Date().toLocaleString('pt-BR')}*
  ...
  const saveData = async (key: string, data: any) => {
    ...
    // (Firestore desativado). A escrita no Supabase substitui este caminho nas
    // tarefas T4 (auth) / T5 (dados do painel e cadastros).
  ...
  const pageDetails: Record<string, { title: string, subtitle: string }> = {
    painel: { title: "Painel do Dia", subtitle: "Confirmações de amanhã, reativações da semana e agenda de hoje — tudo em uma tela." },
  ...
  // já barrou o anônimo no servidor; aqui é só a confirmação no cliente.
  if (sessao === 'carregando' || sessao === 'anonimo') {
    return (
      <SplashScreen
        somenteVisual
        mensagemInicial={sessao === 'anonimo' ? 'Redirecionando para o acesso da equipe...' : 'Abrindo o painel...'}
      />
    );
  }
      ...
      <aside className="sidebar">
        ...
        <div className="sidebar-footer">
          <div className="sessao-card">
            <div className="sessao-avatar" aria-hidden="true">
              {sessao === 'demo' ? <Sunrise size={15} /> : perfil ? inicialDoPerfil(perfil, usuarioEmail) : <User size={15} />}
            </div>
            <div className="sessao-dados">
              <span className="sessao-nome">
                {sessao === 'demo'
                  ? 'Sessão demonstração'
                  : perfil?.nome_completo || usuarioEmail || 'Sessão ativa'}
              </span>
              <span className="sessao-papel">
                {sessao === 'demo' ? 'dados de exemplo' : perfil ? rotuloPapel(perfil.role) : 'carregando perfil...'}
              </span>
            </div>
            <button className="sessao-sair" onClick={() => void sair()} title={sessao === 'demo' ? 'Sair da demonstração' : 'Encerrar sessão'}>
              <LogOut size={15} />
            </button>
          </div>
        ...
        <header className="main-header">
          ...
          <div className="header-actions">
            <div
              ...
              title={
                sessao === 'autenticada'
                  ? 'Sessão autenticada no Supabase. Perfil e RLS ativos; cadastros clínicos ainda em armazenamento local até a T6.'
                  : 'Modo demonstração: dados de exemplo, sem acesso ao banco (a RLS nega tudo para anon).'
              }
              style={{
                display: 'inline-flex',
                alignItems: 'center',
                ...
                fontSize: '0.78rem',
                fontWeight: 600,
                backgroundColor: sessao === 'autenticada' ? 'rgba(16, 185, 129, 0.12)' : 'rgba(234, 179, 8, 0.12)',
                color: sessao === 'autenticada' ? '#10b981' : '#eab308',
                border: sessao === 'autenticada' ? '1px solid rgba(16, 185, 129, 0.35)' : '1px solid rgba(234, 179, 8, 0.35)',
            ...
            >
              <span
                style={{
                  ...
                  height: '8px',
                  borderRadius: '50%',
                  backgroundColor: sessao === 'autenticada' ? '#10b981' : '#eab308',
                  boxShadow: sessao === 'autenticada' ? '0 0 8px #10b981' : '0 0 8px #eab308',
                  display: 'inline-block'
                }}
              />
              <span>{sessao === 'autenticada' ? 'Supabase · sessão ativa' : 'Modo demonstração'}</span>
            </div>
            {sessao === 'autenticada' && usuarioEmail && (
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.35rem 0.8rem', borderRadius: '9999px', border: '1px solid var(--border-color)', fontSize: '0.78rem' }}>
                <User size={13} />
                <span style={{ maxWidth: '180px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {perfil?.nome_completo || usuarioEmail}
                </span>
                {perfil && (
                  <span className="text-muted" style={{ fontSize: '0.72rem' }}>· {rotuloPapel(perfil.role)}</span>
                )}
                <button
                  onClick={() => void sair()}
                  title="Sair da sessão"
                  style={{ display: 'inline-flex', alignItems: 'center', gap: '0.25rem', background: 'transparent', border: 'none', color: '#94a3b8', cursor: 'pointer', fontSize: '0.78rem', padding: 0 }}
                >
                  <LogOut size={13} /> Sair
                </button>
              </div>
            ...
            {sessao === 'demo' && (
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.35rem 0.8rem', borderRadius: '9999px', border: '1px solid rgba(234, 179, 8, 0.35)', fontSize: '0.78rem', background: 'rgba(234, 179, 8, 0.1)' }}>
                <span style={{ color: '#eab308' }}>Sessão demonstração</span>
                <button
                  onClick={() => void sair()}
                  title="Ir para o login"
                  style={{ display: 'inline-flex', alignItems: 'center', gap: '0.25rem', background: 'transparent', border: 'none', color: '#94a3b8', cursor: 'pointer', fontSize: '0.78rem', padding: 0 }}
                >
                  <LogOut size={13} /> Fazer login
                </button>
              </div>
            )}
            ...
            {needsAuth ? (
              <div className="card text-center" style={{ padding: '3.5rem 2rem', maxWidth: '640px', margin: '2rem auto' }}>
                ...
                <h3 style={{ fontSize: '1.5rem', fontFamily: 'var(--font-sans)', fontWeight: 600, marginBottom: '0.75rem' }}>
                  Backup na nuvem — pausado
                </h3>
                <p className="text-muted" style={{ maxWidth: '480px', margin: '0 auto 1.75rem auto', fontSize: '0.95rem', lineHeight: '1.6' }}>
                  A conexão com o Google Drive via login Google (Firebase Auth) foi desativada durante a migração para o Supabase. A autenticação retorna com o Supabase Auth e o backup na nuvem será reavaliado na nova arquitetura.
                </p>
